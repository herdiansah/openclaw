...
# OPENCLAW_QDRANT_MEMORY_ROOT overrides where MEMORY.md and memory/ are read from.
# Defaults to ROOT_DIR for backwards compatibility.
MEMORY_ROOT="${OPENCLAW_QDRANT_MEMORY_ROOT:-$ROOT_DIR}"
...
fallback_native() {
  ...
  local max_hits="$2"
  local files=()
  [[ -f "$MEMORY_ROOT/MEMORY.md" ]] && files+=("$MEMORY_ROOT/MEMORY.md")
  if [[ -d "$MEMORY_ROOT/memory" ]]; then
    while IFS= read -r f; do files+=("$f"); done < <(find "$MEMORY_ROOT/memory" -maxdepth 1 -type f -name '*.md' | sort)
  fi
