...
// OPENCLAW_QDRANT_MEMORY_ROOT overrides the base directory for MEMORY.md and
// memory/*.md files.  Defaults to ROOT_DIR for backwards compatibility.
const MEMORY_ROOT = process.env.OPENCLAW_QDRANT_MEMORY_ROOT
  ? path.resolve(process.env.OPENCLAW_QDRANT_MEMORY_ROOT)
  : ROOT_DIR;
const MEMORY_DIR = path.join(MEMORY_ROOT, "memory");
// State file stays inside the openclaw repo dir (internal bookkeeping, not user memory)
const STATE_FILE = path.join(ROOT_DIR, "memory", "qdrant-index-state.json");
...
async function main() {
  ...
  if (memoryFiles.length) {
    ...
    for (const file of memoryFiles) {
      const source = path.relative(MEMORY_ROOT, file);
